...
# take valid indices
indValid = np.logical_and((ranges < 10),(ranges> lidar_range_min))
...
for times_exec in range(1):  
    for i in tqdm(range(len(v_timestep)-1)):
        ...
        trajectory[:,i+1] = nextState(trajectory[:,i].T,vel[i],wz[i],
                                        ...
                                        trajectory[:,i,2].T).T
        trajectory[:,i+1] = trajectory[:,i+1] + np.append(np.random.normal(0.0, 0.001, (N_PARTICLES*2)),
        ...
        if (i%100 == 0):
            ...
            np.save('MAP' + str(i) + '.npy', np.array(MAP_COPY, dtype=object), allow_pickle=True)
    ...
    # take valid indices
        ...
        for particle_index in range(N_PARTICLES):
            ...
            x_range = np.array([xp])
            y_range = np.array([yp])
            corr = mapCorrelation(MAP_COPY,x_im,y_im,YY,x_range,y_range)[0,0]
            
            
            if (corr<1):
                corr = 1
            
            prob[particle_index] = prob[particle_index]*corr
