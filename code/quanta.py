...
for j in range(len(xi0)):
    ...
    MAP_COPY[XY[0].astype(int),XY[1].astype(int)] = 0
    MAP_COPY[epx,epy] = 1



for times_exec in range(1):  
    for i in tqdm(range(len(v_timestep)-1)):
        
        trajectory[:,i+1] = nextState(trajectory[:,i].T,vel[i],wz[i],
                                        v_timestep[i+1] - v_timestep[i],
                                        trajectory[:,i,2].T).T
        trajectory[:,i+1] = trajectory[:,i+1] + np.append(np.random.normal(0.0, 0.005, (N_PARTICLES*2)),
    ...
    # take valid indices
        indValid = np.logical_and((ranges < lidar_range_max),(ranges> lidar_angle_min))
        ranges = ranges[indValid]
        ...
        if (i%100 == 0):
            ...
            np.save('MAP1' + str(i) + '.npy', np.array(MAP_COPY, dtype=object), allow_pickle=True)
        ...
        for particle_index in range(N_PARTICLES):
            ...
            corr = mapCorrelation(MAP['map'],x_im,y_im,YY,x_range,y_range)
            max_index_corr = np.unravel_index(np.argmax(corr),corr.shape)
            if (corr[max_index_corr]<1):
                corr[max_index_corr] = 1
            
            trajectory[particle_index,i+1,0] = x_range[max_index_corr[0]] + xp
            trajectory[particle_index,i+1,1] = y_range[max_index_corr[1]] + yp
            prob[particle_index] = prob[particle_index]*corr[max_index_corr]
        ...
        for j in range(len(xi0)):
            ...
            spx = xip
            spy = yip
            if(spx>=MAP['sizex']-1): spx = MAP['sizex']-1
            if(spx<=0): spx = 0
            if(spy>=MAP['sizey']-1): spy = MAP['sizey']-1
            if(spy<=0): spy = 0
            #print(i,angles[j])
            #print(round(spx),round(spy),round(epx),round(epy))
            XY = bresenham2D(spx, spy, epx, epy)
...
plt.figure(5)
plt.imshow(MAP_COPY,cmap="gist_yarg") 
plt.show() 
