...
for times_exec in range(1):  
    for i in tqdm(range(len(v_timestep)-1)):
        ...
        trajectory[:,i+1] = nextState(trajectory[:,i].T,vel[i],wz[i],
                                        ...
                                        trajectory[:,i,2].T).T
        trajectory[:,i+1] = trajectory[:,i+1] + np.append(np.random.normal(0.0, 0.05, (N_PARTICLES*2)),
    ...
    # take valid indices
        ...
        for particle_index in range(N_PARTICLES):
            ...
            x0 = ranges*np.cos(angles_local)
            y0 = ranges*np.sin(angles_local) 
            YY = np.stack((x0,y0))
            x_range = np.array([xp])
            y_range = np.array([yp])
            corr = mapCorrelation(MAP['map'],x_im,y_im,YY,x_range,y_range)[0,0]
